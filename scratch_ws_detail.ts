...
import {
  ...
  Eye, History, Send, Loader2, ClipboardList, Pen, Network,
} from 'lucide-react';
        ...
        <div className="px-8 py-5">
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              <Button variant="outline" size="sm" className="h-7 text-xs bg-indigo-50 text-indigo-700 border-indigo-200 hover:bg-indigo-100" onClick={() => setWorkflowDialogOpen(true)}>
                <Network className="w-3 h-3 mr-1.5" /> ดูขั้นตอน (Workflow)
              </Button>
            </div>
          </div>
        </div>
            ...
            <Card className="border-gray-100"><CardContent className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                {Object.entries(task.payload).map(([key, val]) => {
                  ...
                  return (
                    <div key={key} className={cn("py-2 border-b border-gray-50", isLongText ? "col-span-1 md:col-span-2" : "")}>
                      <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1">{key}</p>
                      <p className={cn("text-sm font-medium text-gray-800", isLongText ? "leading-relaxed" : "")}>
                        {typeof val === 'boolean' ? (val ? '✓ ใช่' : '✗ ไม่ใช่') : String(val ?? '-')}
                      </p>
                    </div>
                  );
                })}
        ...
        <DialogContent className="sm:max-w-md p-0 overflow-hidden">
          <div className="bg-gradient-to-r from-indigo-600 to-blue-700 px-6 py-5 text-white">
            <DialogTitle className="text-white text-lg flex items-center gap-2">
              ...
              ขั้นตอนการพิจารณา (Workflow)
            </DialogTitle>
            <DialogDescription className="text-indigo-100 mt-1">
              แสดงสายพานการอนุมัติสำหรับคำขอนี้
            </DialogDescription>
          </div>
          <div className="p-6">
            <div className="relative border-l-2 border-indigo-100 ml-3 space-y-8 pb-4">
              <div className="relative">
                <div className="absolute -left-[29px] top-1 w-4 h-4 rounded-full bg-emerald-500 ring-4 ring-white" />
                <div className="pl-6">
                  <p className="text-sm font-bold text-gray-900">ยื่นคำร้อง</p>
                  <p className="text-xs text-gray-500 mt-1">ผู้ยื่น: นทร. / วันที่: 25/04/2569</p>
                </div>
              </div>
              <div className="relative">
                <div className="absolute -left-[29px] top-1 w-4 h-4 rounded-full bg-emerald-500 ring-4 ring-white" />
                <div className="pl-6">
                  <p className="text-sm font-bold text-gray-900">ตรวจสอบความถูกต้องเอกสาร</p>
                  <p className="text-xs text-gray-500 mt-1">ผู้รับผิดชอบ: เจ้าหน้าที่ สนร. (เสร็จสิ้น)</p>
                </div>
              </div>
              <div className="relative">
                <div className="absolute -left-[29px] top-1 w-4 h-4 rounded-full bg-blue-500 ring-4 ring-blue-50 animate-pulse" />
                <div className="pl-6 bg-blue-50/50 p-3 rounded-lg border border-blue-100 mt-[-8px]">
                  <p className="text-sm font-bold text-blue-700">พิจารณาอนุมัติเบื้องต้น</p>
                  <p className="text-xs text-blue-600 mt-1">ผู้รับผิดชอบ: อัครราชทูตที่ปรึกษา (กำลังดำเนินการ)</p>
                </div>
              </div>
              <div className="relative opacity-50">
                <div className="absolute -left-[29px] top-1 w-4 h-4 rounded-full bg-gray-300 ring-4 ring-white" />
                <div className="pl-6">
                  <p className="text-sm font-bold text-gray-700">พิจารณาอนุมัติขั้นสุดท้าย</p>
                  <p className="text-xs text-gray-500 mt-1">ผู้รับผิดชอบ: ศูนย์นักเรียนทุน ก.พ.</p>
                </div>
              </div>
            </div>
            <div className="mt-4 flex justify-end">
              <Button variant="outline" onClick={() => setWorkflowDialogOpen(false)}>ปิด</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
